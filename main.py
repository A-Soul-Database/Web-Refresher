...
def Open_Tasks(tag:str,nums:int):
    driver = webdriver.Chrome(chrome_options=chrome_options,executable_path="./GoogleChromePortable/chromedriver.exe")
    times = 0
    while True:
        times+=1
        driver.get(f"https://s.weibo.com/weibo?q={parse.quote(tag)}")
        time.sleep(3)
        os.system(f'echo "{round(time.time()-Start_Time,2)}s : 进程ID: {nums} 打开了, 标题: {driver.title}"')
...
if __name__ == '__main__':
    ...
    for i in Config["Tags"]:
        for n in range(Config["Process_Nums"]):
            Process(target=Open_Tasks,args=(i,n)).start()
